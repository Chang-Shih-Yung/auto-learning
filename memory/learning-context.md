...
2. **整理與消化** — 分兩個子步驟：
   ...
   **⚠️ 工具引用驗證（寫作前必做，適用於所有提及具體工具的文章）**
   文章若涉及具體工具（npm 套件、CLI、GitHub repo），在下筆前必須確認以下任一條件成立：
   - GitHub stars **> 500**，或 npm weekly downloads **> 10,000**

   | 結果 | 寫法 |
   |------|------|
   | 通過門檻 | 正常寫，可以給具體指令與安裝範例 |
   | 未達門檻 | 只描述概念與設計思路，**不給安裝指令、不寫具體命令**，文末加一行：`> ⚠️ 目前 X stars，新工具，使用前請自行確認安全性。` |

   例外：若工具是 Anthropic / Google / Meta / Microsoft 官方發布（有官方背書），可豁免 stars 門檻，但仍須標注版本號。
